...
Todo:
    - Add support for more than just RNNCell       
...
class DeepSpeech2(TensorflowModel):
    def _construct(self):
        ...
        with tf.variable_scope('deepspeech_conv2') as scope:
            ...
            l2_stride = [1,2,1,1]
            conv2 = tf.nn.conv2d(conv1, l2_filter, l2_stride, padding='SAME')
        ...
        with tf.variable_scope('deepspeech_conv3') as scope:
            ...
            l3_stride = [1,2,1,1]
            conv3 = tf.nn.conv2d(conv2, l3_filter, l3_stride, padding='SAME')
        ...
        with tf.variable_scope('deepspeech_recurrent1') as scope:
            r1_cell = tf.nn.rnn_cell.BasicRNNCell(256)
            recurrent1 = tf.nn.dynamic_rnn(r1_cell, conv3, sequence_length=self._batch_size, time_major=True, dtype=tf.float32)
        ...
        with tf.variable_scope('deepspeech_recurrent2') as scope:
            r2_cell = tf.nn.rnn_cell.BasicRNNCell(256)
            recurrent2 = tf.nn.dynamic_rnn(r2_cell, recurrent1, sequence_length=self._batch_size, time_major=True, dtype=tf.float32)
        ...
        with tf.variable_scope('deepspeech_recurrent3') as scope:
            r3_cell = tf.nn.rnn_cell.BasicRNNCell(256)
            recurrent3 = tf.nn.dynamic_rnn(r3_cell, recurrent2, sequence_length=self._batch_size, time_major=True, dtype=tf.float32)
        ...
        with tf.variable_scope('deepspeech_recurrent4') as scope:
            r4_cell = tf.nn.rnn_cell.BasicRNNCell(256)
            recurrent4 = tf.nn.dynamic_rnn(r4_cell, recurrent3, sequence_length=self._batch_size, time_major=True, dtype=tf.float32)
